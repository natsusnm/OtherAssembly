...
namespace PRADA_Vayne.MyUtils
{
    public static class Extensions
    {
        ...
        public static bool IsCondemnable(this AIHeroClient hero)
        {
            ...
            var p = hero.Position;
            var pD = Program.ComboMenu["EPushDist"].GetValue<MenuSlider>().Value;
            var mode = Program.ComboMenu["EMode"].GetValue<MenuList>().SelectedValue;

            if (mode == "PRADASMART" && (p.Extend(pP, -pD).IsCollisionable() || p.Extend(pP, -pD / 2f).IsCollisionable() || p.Extend(pP, -pD / 3f).IsCollisionable()))
            {
                ...
                if (enemiesCount > 1 && enemiesCount <= 3)
                {
                    ...
                    {
                        var posFlags = NavMesh.GetCollisionFlags(prediction.UnitPosition.ToVector2().Extend(pP.ToVector2(), -i).ToVector3());
                ...
                var hitchance = Program.ComboMenu["EHitchance"].GetValue<MenuSlider>().Value;
                ...
                for (var i = 15; i < pD; i += 100)
                    if (pP.ToVector2().Extend(alpha, i).ToVector3().IsCollisionable() && pP.ToVector2().Extend(beta, i).ToVector3().IsCollisionable())
                        return true;
                return false;
            }

            if (mode == "PRADAPERFECT" && (p.Extend(pP, -pD).IsCollisionable() || p.Extend(pP, -pD / 2f).IsCollisionable() || p.Extend(pP, -pD / 3f).IsCollisionable()))
            {
                if (!hero.CanMove || hero.Spellbook.IsAutoAttack)
                    return true;
            ...
            {
                var prediction = Program.E.GetPrediction(hero);
                return NavMesh.GetCollisionFlags(prediction.UnitPosition.ToVector2().Extend(pP.ToVector2(), -pD).ToVector3()).HasFlag(CollisionFlags.Wall) || NavMesh.GetCollisionFlags(prediction.UnitPosition.ToVector2().Extend(pP.ToVector2(), -pD / 2f).ToVector3()).HasFlag(CollisionFlags.Wall);
            }
            ...
            if (mode == "FASTEST" && (p.Extend(pP, -pD).IsCollisionable() || p.Extend(pP, -pD / 2f).IsCollisionable() || p.Extend(pP, -pD / 3f).IsCollisionable()))
                return true;

            var j4 = ObjectManager.Get<AIHeroClient>().FirstOrDefault(h => h.IsEnemy && h.CharacterName == "JarvanIV" && h.Distance(ObjectManager.Player) < 800);
            if (j4 != null)
            {
                if (ObjectManager.Get<AIMinionClient>().Any(m => m.CharacterName == "jarvanivwall" && m.Distance((Vector2)pP.Extend(p, 425)) < 100))
                    return true;
                if (!j4.IsDead && ObjectManager.Get<AIMinionClient>().Any(m => m.CharacterName == "jarvanivwall" && m.Distance(j4.Position.Extend(p, 425)) < 100)) Program.E.Cast(j4);
            }
        ...
        public static Vector3 GetTumblePos(this AIBaseClient target)
        {
            ...
            if (Program.ComboMenu["QOrderBy"].GetValue<MenuList>().SelectedValue == "CLOSETOTARGET")
        ...
        {
            return GameObjects.EnemyHeroes.Any(e => e.IsValidTarget() && e.IsVisible && e.Distance(pos) < Program.ComboMenu["QMinDist"].GetValue<MenuSlider>().Value) || Traps.EnemyTraps.Any(t => pos.Distance(t.Position) < 125) || pos.IsUnderEnemyTurret() && !Player.IsUnderEnemyTurret() || pos.IsWall();
        }
