...
namespace PRADA_Vayne.MyLogic.E
{
    public static partial class Events
    {
        public static void OnProcessSpellCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            ...
            if (args.Target != null && args.Target.IsMe || args.End.DistanceToPlayer() < 350 * 350)
            {
            ...
            if (sender.Distance(ObjectManager.Player) < 550)
            {
                ...
                if (casterName == "MasterYi" && slot == SpellSlot.W)
                    ...
                    {
                        var flags = NavMesh.GetCollisionFlags(sender.Position.ToVector2().Extend(Heroes.Player.Position.ToVector2(), -i).ToVector3());

                        if (flags != null && (flags == CollisionFlags.Grass || flags == CollisionFlags.Building))
                        {
                            Program.E.Cast(sender);
                            return;
                        }
